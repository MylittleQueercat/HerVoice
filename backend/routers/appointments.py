...
from services.appointment_service import book_appointment, cancel_appointment, reschedule_appointment
...
def update(appointment_id: str, request: UpdateAppointmentRequest, db: Session = Depends(get_db)):
    ...
    try:
        if request.action == "cancel":
            appointment = cancel_appointment(request.access_code, db)
        elif request.action == "reschedule":
            if not request.new_slot_id:
                raise ValueError("new_slot_id is required for reschedule")
            appointment = reschedule_appointment(request.access_code, request.new_slot_id, db)
        else:
